...
% \begin{verbatim}
%   \documentclass[<option>]{assignments}
% 
%   \course{}
%   \group{}
%   \due{}
...
% \texttt{article} understands. Additionally, pass the option \texttt{sansserif}
% to use \texttt{helvet} as the default font. I would argue that in the age of
% assignments being handed in digitally and almost always being reviewed digitally
% as well, sans-serif fonts should be used as per their recommendation for digital
% screens, but then again, nowadays most displays have a higher pixel density than most
% printers and operating systems better rasterization implementations, such that serifs
% are no longer ugly artifacts. Therefore, the sans-serif default font is only optional,
% rather \texttt{tgpagella} is used, because it improves upon the already excellent 
% URW Palladio, which in turn is a free reinterpretation of the Palatino font face.
%
% \section{Required Packages}
%
% Notably, the class introduces several dependencies. Here's a list of the
% packages required by \texttt{assignments}:
%
% \begin{description}
...
%   \item[helvet] \texttt{CTAN:macros/latex/required/psnfss}
% \end{description}
...
% \texttt{assignments} comes with several predefined prefixes that are localized in both 
% English and German:
% \begin{macrocode}
...
% \end{macrocode}
%
% \begin{macro}{\exercise\oarg{exercise points}\marg{exercise title}}
% \begin{macro}{\exercise*\oarg{exercise points}\marg{exercise title}} 
...
% points stored in the first run. \textbf{This is the reason assignments
...
% The \texttt{assignments} class offers several customizations to the default title of article.
...
%
% \texttt{assignments} includes several frequently used macros for mathmode, that were,
...
% see \(\mathbb{Z}/\n\:\), which even works in regular text due to custom trailing spacing.
% \begin{macrocode}
...
%
% \begin{macro}{\divides}
% \begin{macro}{\property}
% \cmd{\divides} and \cmd{\property} do the same thing under the hood, while having different
% semantics. \cmd{\divides} denotes the binary relation of \(a\) divides \(b\), whereas 
% \cmd{\property} should be used to separate declaration from restrictions in a definition of a set,
% namely the vertical bar in \(\{x \ |\ x \in \mathbb{R}\}\).
% \begin{macrocode}
